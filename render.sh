...
TEMPORAL_ITERATIONS="${TEMPORAL_ITERATIONS:-500}"
...
for frame in $(seq "${START_FRAME}" "${END_FRAME}"); do
  ...
  policy_path="${POLICY_PATH:-${model_path}/${ALLOC_POLICY}_${TOTAL_SPLATS}.npy}"
  if [[ ! -f "${policy_path}" && -f "${OUTPUT}/${ALLOC_POLICY}_${TOTAL_SPLATS}.npy" ]]; then
